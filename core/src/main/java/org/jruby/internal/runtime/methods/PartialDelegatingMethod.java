...
public class PartialDelegatingMethod extends DynamicMethod {
    ...
    // MRI: vm_call0_body and aliased_callable_entry.method_entry
    /* FIXME: This is not quite right. It appears that MRI does this logic at call time, but I believe
              the calculated class is cached somewhere along with the "callable" entry.method entry. The code
              below means all aliases in modules will do the implementer search, unless we can detect
              that the related entry.method does not need "super". We can improve this at cache time in either
